...
if [ -z "$(find "$CONFIG_DIR" -name '*.ovpn' -print -quit)" ]; then
...
if [ -z "$OVPN_CREDENTIALS" ]; then
  ...
  openvpn --config "$OVPN_FILE" 2>&1 | tee /var/log/ovpn.log &
elif [ -f "$OVPN_CREDENTIALS" ]; then
  log "INFO:" "Using credentials file: $OVPN_CREDENTIALS"
  openvpn --config "$OVPN_FILE" --auth-user-pass "$OVPN_CREDENTIALS" 2>&1 | tee /var/log/ovpn.log &
else
...
get_tun_ip() {
  ip -4 addr show tun0 | grep inet | awk '{print $2}' | cut -d/ -f1
}
...
for i in $(seq 0 $((INIT_TIMEOUT - 1))); do
  if grep -q "Initialization Sequence Completed" /var/log/ovpn.log 2>/dev/null; then
    break
  fi
  sleep 1
done
...
/usr/bin/tinyproxy -d -c /tmp/tinyproxy.conf.1 &
PROXY_PID=$!
